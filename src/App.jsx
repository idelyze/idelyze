...
export default function App() {
  ...
  const [dark, setDark] = useState(() => {
    const saved = localStorage.getItem('idelyze-theme');

    // Manual override always wins
    if (saved) return saved === 'dark';

    // Otherwise auto-switch using local time
    return getAutoTheme();
  });
  ...
  useEffect(() => {
    ...
    const syncTheme = () => {
      ...
      // If user manually selected a theme,
      // don't auto-change it.
      if (saved) return;

      setDark(getAutoTheme());
    };
  ...
  const toggleDark = () => {
    setDark((d) => {
      const next = !d;
      localStorage.setItem('idelyze-theme', next ? 'dark' : 'light');
      return next;
    });
  };

  const resetThemeToAuto = () => {
    localStorage.removeItem('idelyze-theme');
    setDark(getAutoTheme());
  };

  return (
    <ThemeCtx.Provider value={{ T, dark, toggleDark }}>
              ...
              <m.div
                ...
                <BrowserRouter>
                  ...
                  <div style={{ background: T.bg, minHeight: "100vh", transition: "background 0.3s ease" }}>
                    <Navbar T={T} dark={dark} toggleDark={toggleDark} />
